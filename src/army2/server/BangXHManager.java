...
public class BangXHManager {
    ...
    public static void init() {
        ...
        /*
         * Calendar cl = GregorianCalendar.getInstance(); Date d = new Date();
         * cl.setTime(d); cl.set(Calendar.HOUR_OF_DAY, 0); cl.set(Calendar.MINUTE, 0);
         * cl.set(Calendar.SECOND, 0); cl.add(Calendar.DATE, 1); t.schedule(new
         * TimerTask() {
         */
        // @Override
        // public void run() {
        System.out.println("Update BXH");
        ...
        switch (type) {
            case 0:
                try {
                    ...
                    ResultSet red = SQLManager.stat
                            .executeQuery("SELECT `id`,`dvong` FROM `armymem` ORDER BY `dvong` DESC;");
                    while (red.next()) {
                        ...
                        // SQLManager.stat.executeUpdate("UPDATE `armymem` SET `top`='"+i+"' WHERE
                        // `id`="+iddb+" LIMIT 1;");
                        i++;
                    }
                    red.close();
            ...
            case 3:
                try {
                    int i = 0;
                    ResultSet red = SQLManager.stat
                            .executeQuery("SELECT `id`,`luong` FROM `armymem` ORDER BY `luong` DESC LIMIT 0, 100;");
                    while (red.next()) {
                        int iddb = red.getInt("id");
