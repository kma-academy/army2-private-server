...
public class ServerManager {
    ...
    public static User getUser(int iddb) {
        ...
        try {
            ResultSet red = SQLManager.stat
                    .executeQuery("SELECT `idapp`,`online` FROM `armymem` WHERE id=\"" + iddb + "\" LIMIT 1;");
            if (red.first()) {
    ...
    protected static void init() {
        ...
        System.out.println("Cap nhat nap the");
        try {
    ...
    protected static void start() {
        System.out.println("Start socket port=" + post);
        try {
                ...
                for (int i = 0; i < bxhA.length; i++) {
                    try {
                        ...
                        red = SQLManager.stat.executeQuery(
                                "SELECT `NVused`,`clan` FROM `armymem` WHERE id=\"" + iddb + "\" LIMIT 1;");
                        red.first();
                        byte nv = 0;
    ...
    protected static void napTheMessage(User us, Message ms) throws IOException {
        /*
         * DataOutputStream ds; ms = new Message(46); ds = ms.writer();
         * ds.writeUTF(String.format("PuaruVN")); ds.flush(); us.sendMessage(ms);
         */
